...
mcmc_result_inference_t inference_cppWithProposal(
    std::vector<size_t> demography, std::vector<size_t> age_group_limits,
    Eigen::MatrixXi ili, Eigen::MatrixXi mon_pop, Eigen::MatrixXi n_pos,
    Eigen::MatrixXi n_samples, flu::vaccine::vaccine_t vaccine_calendar,
    Eigen::MatrixXi polymod_data, const Eigen::VectorXd &initial,
    const Eigen::VectorXd &initial_contact_ids,
    proposal::proposal_state_t proposal_state, Eigen::MatrixXd mapping,
    Eigen::VectorXd risk_ratios, Eigen::VectorXd epsilon_index,
  ...
  age_data.age_group_sizes =
      flu::data::group_age_data(demography, age_group_limits);

  auto pop_vec = flu::data::stratify_by_risk(age_data.age_group_sizes,
  ...
  for (int i = 0; i < mapping.rows(); ++i) {
    // to_j = weight*from_i
    pop_RCGP((size_t)mapping(i, 1)) +=
        mapping(i, 2) * pop_vec((size_t)mapping(i, 0));
  }
  ...
  *********************************************************************************************************************************************************/
  ...
  auto pars_to_susceptibility =
      [&susceptibility_index](const Eigen::VectorXd &pars) {
  ...
  auto polymod =
      flu::contacts::table_to_contacts(polymod_data, age_group_limits);

  auto curr_c = contacts::shuffle_by_id(polymod, contact_ids);
  ...
  auto curr_llikelihood = log_likelihood_hyper_poisson(
      ...
      days_to_weeks_AG(result, mapping, pop_RCGP.size()), ili, mon_pop, n_pos,
      n_samples, pop_RCGP, abs_err);
  ...
  auto log_prior_ratio_f =
      [pass_prior, &Rlprior, &prop_prior, &curr_prior, uk_prior, &epsilon_index,
       psi_index, transmissibility_index, &susceptibility_index,
       initial_infected_index](const Eigen::VectorXd &proposed,
        ...
        if (uk_prior)
          ...
          for (auto i = 0; i < susceptibility_index.size(); ++i) {
            ...
            if (!std::isfinite(prop_prior) || proposed[index] < 0 ||
                proposed[index] > 1) {
              prop_prior = log(0);
              break;
            }
  ...
  **************************************************************************************************************************************************************/
  ...
  while (sampleCount < nbatch) {
    ...
    proposal_state =
        proposal::update(std::move(proposal_state), curr_parameters, k);

    /*
    if (k>=nburn)
    {
      Rcpp::Rcout << "Adaptive scaling: " << proposal_state.adaptive_scaling <<
    std::endl; Rcpp::Rcout << "past_acceptance: " <<
    proposal_state.past_acceptance << std::endl; Rcpp::Rcout << "conv_scaling: "
    << proposal_state.conv_scaling << std::endl; Rcpp::Rcout << "Acceptance: "
    << proposal_state.acceptance << std::endl; Rcpp::Rcout <<
    proposal_state.emp_cov_matrix << std::endl << std::endl;
    }
    */
    /*
    ...
    if (!std::isfinite(prior_ratio)) {
      ...
      proposal_state = proposal::accepted(std::move(proposal_state), false, k);
    } else {
      /*translate into an initial infected population*/
      ...
      if (no_risk_groups < 3) {
        ...
        for (size_t i = no_age_groups * no_risk_groups;
             i < prop_init_inf.size(); ++i) {
          prop_init_inf[i] = 0;
        }
      }
      auto prop_c = curr_c;

      /*do swap of contacts step_mat times (reduce or increase to change
       * 'distance' of new matrix from current)*/
      ...
      if (R::runif(0, 1) < p_ac_mat)
        prop_c =
            contacts::bootstrap_contacts(std::move(prop_c), polymod, step_mat);
      ...
      prop_likelihood += log_likelihood_hyper_poisson(
          pars_to_epsilon(prop_parameters), prop_parameters[psi_index],
          days_to_weeks_AG(result, mapping, pop_RCGP.size()), ili, mon_pop,
          n_pos, n_samples, pop_RCGP, abs_err);

      /*Acceptance rate include the likelihood and the prior but no correction
       * for the proposal as we use a symmetrical RW*/
      // Make sure accept works with -inf prior
      // MCMC-R alternative prior?
      if (std::isinf(prop_likelihood) && std::isinf(curr_llikelihood))
        my_acceptance_rate = exp(prior_ratio);  // We want to explore and find a
                                                // non infinite likelihood
      else
        my_acceptance_rate =
            exp(prop_likelihood - curr_llikelihood + prior_ratio);

      if (R::runif(0, 1) < my_acceptance_rate) /*with prior*/
      {
        /*update the acceptance rate*/
        proposal_state = proposal::accepted(std::move(proposal_state), true, k);
      ...
      {
        proposal_state =
            proposal::accepted(std::move(proposal_state), false, k);
      }
    }
  }
  return results;
...
mcmc_result_inference_t inference_cppWithCovariance(
    std::vector<size_t> demography, std::vector<size_t> age_group_limits,
    Eigen::MatrixXi ili, Eigen::MatrixXi mon_pop, Eigen::MatrixXi n_pos,
    Eigen::MatrixXi n_samples, flu::vaccine::vaccine_t vaccine_calendar,
    Eigen::MatrixXi polymod_data, Eigen::VectorXd initial_parameters,
    Eigen::VectorXd initial_contact_ids, Eigen::VectorXd means,
    Eigen::MatrixXd covariance, size_t covariance_weight,
    Eigen::MatrixXd mapping, Eigen::VectorXd risk_ratios,
    Eigen::VectorXd epsilon_index, size_t psi_index,
  ...
  return inference_cppWithProposal(
      demography, age_group_limits, ili, mon_pop, n_pos, n_samples,
      vaccine_calendar, polymod_data, initial_parameters, initial_contact_ids,
...
mcmc_result_inference_t inference_cpp(
    std::vector<size_t> demography, std::vector<size_t> age_group_limits,
    Eigen::MatrixXi ili, Eigen::MatrixXi mon_pop, Eigen::MatrixXi n_pos,
    Eigen::MatrixXi n_samples, flu::vaccine::vaccine_t vaccine_calendar,
    Eigen::MatrixXi polymod_data, Eigen::VectorXd initial,
    Eigen::MatrixXd mapping, Eigen::VectorXd risk_ratios,
    Eigen::VectorXd epsilon_index, size_t psi_index,
    size_t transmissibility_index, Eigen::VectorXd susceptibility_index,
    size_t initial_infected_index, Rcpp::Function lprior, bool pass_prior,
